...
impl<const N: usize> OdometryTask<N> {
    ...
    fn update(&mut self, new_angle: Angle) {
        ...
        let angle_change = self.last_angle - new_angle; // TODO: Wrap this properly
    ...
    pub async fn entrypoint(mut self) {
        loop {
            ...
            let angle = HEADING_SIGNAL.wait().await;
...
pub async fn start_odometry_task(task: OdometryTask<2>) {
    task.entrypoint().await;
}
